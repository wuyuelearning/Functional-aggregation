...
/**
 * 博客中有 setTint  但是这里会报错
 *
 * Created by wuyue on 2019/9/11.
 * description:
 */
public class TabView extends FrameLayout {
    private ImageView mNormalImageView; //代表默认显示的轮廓图
    private ImageView mSelectedImageView; //代表选中后的图
    private TextView mTitleView;

    private String mTitle;   // 代表要显示的标题
    private Drawable mNormalDrawable;
    private Drawable mSelectedDrawable;
    private int mTargetColor;  //代表变色最终显示的颜色，这个颜色可以从选中图中用取色器获取。
    ...
    protected void onFinishInflate() {
        ...
        // 设置轮廓图片，不透明，默认着色为黑色
        mNormalImageView =(ImageView) findViewById(R.id.tab_image);
//        mNormalDrawable.setTint(DEFAULT_TAB_COLOR);  // ！！！！！博客中有，但这里会报错
        mNormalDrawable.setAlpha(255);
        mNormalImageView.setImageDrawable(mNormalDrawable);

        // 设置选中图片，透明， 默认着色为黑色
        mSelectedImageView = (ImageView)findViewById(R.id.tab_image_top);
...
//        mNormalDrawable.setTint(finalColor);  // ！！！！！博客中有，但这里会报错
        // 2. 透明度变换
